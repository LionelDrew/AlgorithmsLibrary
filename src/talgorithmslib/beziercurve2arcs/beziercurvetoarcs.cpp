                                                                     ...
                                                                     qreal iStartT, qreal iEndT, double iAllowableError)
{
    ...
    QPointF G = BezierMathTools::findInCenterPointOfTriangle(A0, V, A1);
    ...
    QPointF H = BezierMathTools::calculateUnitTangentVectorOfCircle(center1, G);
...
double BezierCurveToArcs::findTWithNewtonAndRaphsonMethod(QPointF A, QPointF controlPointA, QPointF controlPointB, QPointF B, QPointF H, QPointF G, double allowableError, double startT, double endT) {
    ...
    double fn = BezierMathTools::dotProductOfTwoPoints(Q_tn, H)
            - BezierMathTools::dotProductOfTwoPoints(G, H);
    ...
    while (abs(fn) > allowableError) {
        ...
        double d_fn = BezierMathTools::dotProductOfTwoPoints(d_Q_tn, H);
        ...
        fn = BezierMathTools::dotProductOfTwoPoints(Q_tn, H)
                - BezierMathTools::dotProductOfTwoPoints(G, H);
    }
...
QPointF BezierCurveToArcs::calculateV(QPointF newA, QPointF newB, QPointF A, QPointF controlPointA, QPointF controlPointB, QPointF B, double startT, double endT) {
    ...
    return BezierMathTools::calculateIntersectionOfTwoLine(newA, adjacentNewA, adjacentNewB, newB);
}
...
QPointF BezierCurveToArcs::calculateCenterOfArc(QPointF A, QPointF V, QPointF G) {
    ...
    return BezierMathTools::solveLinearEquationsOfTwoUnknownVariables(
...
double BezierCurveToArcs::calculateMaxError(QPointF A, QPointF controlPointA, QPointF controlPointB, QPointF B, QPointF G, double t) {
    ...
    return BezierMathTools::euclideanDistance(Q_t, G);
}

Arc BezierCurveToArcs::generateArc(QPointF center, QPointF startPoint, QPointF endPoint) {

    double radius = BezierMathTools::euclideanDistance(center, endPoint);

    double startAngle
            = BezierMathTools::calculateAngleRelativeToCircleCenter(center, startPoint);
    double endAngle
            = BezierMathTools::calculateAngleRelativeToCircleCenter(center, endPoint);
